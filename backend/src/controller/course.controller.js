const AsyncHandler = require("express-async-handler");
const myCourse = require("../models/course.model");
const ApiResponse = require("../utils/ApiResponse.utils");
const ErrorHandler = require("../utils/ErrorHandler.utils");
const cloudinary = require("cloudinary");
const fs = require("fs").promises;
const path = require("path");

// &-----------------------------Create Course-----------------------------

const createCourse = AsyncHandler(async (req, res, next) => {
  const { title, description, category, createdBy } = req.body;

  if (!title || !category || !description || !createdBy) {
    return next(new ErrorHandler("All fields are required", 400));
  }

  const course = await myCourse.create({
    title,
    description,
    category,
    createdBy,
    thumbnail: {
      public_id: "Dummy",
      secure_url: "Dummy",
    },
  });

  if (!course) {
    return next(
      new ErrorHandler("Course could not be created, please try again", 400),
    );
  }

  // Run only if user sends a file
  if (req.file) {
    try {
      const result = await cloudinary.v2.uploader.upload(req.file.path, {
        folder: "lms", // Save files in a folder named lms
      });

      //  If success
      if (result) {
        // Set the public_id and secure_url in array
        course.thumbnail.public_id = result.public_id;
        course.thumbnail.secure_url = result.secure_url;
      }
      // After successful upload remove the file from local storage
      fs.rm(`uploads/${req.file.filename}`);
    } catch (error) {
      // Empty the uploads directory without deleting the uploads directory

      for (const file of await fs.readdir("uploads/")) {
        await fs.unlink(path.join("uploads/", file));
      }

      // Send the error message
      return next(
        new ErrorHandler(
          JSON.stringify(error) || "ile not uploaded, please try again",
          400,
        ),
      );
    }
  }

  // Save the changes
  await course.save();

  new ApiResponse(200, true, "Course created successfully", course).send(res);
});

// & -----------------------------GetAllCourses---------------------------
const getAllCourses = AsyncHandler(async (_req, res, next) => {
  // Find all the courses without lectures
  const courses = await myCourse.find({}).select("-lectures");

  res.status(200).json({
    success: true,
    message: "All courses",
    courses,
  });
});

// & --------------------------UpdateCourseById----------------------------
const updateCourseById = AsyncHandler(async (req, res, next) => {
  // Extracting the course id from the request params
  const { id } = req.params;

  // Finding the course using the course id
  const course = await myCourse.findByIdAndUpdate(
    id,
    {
      $set: req.body, // This will only update the fields which are present
    },
    {
      runValidators: true, // This will run the validation checks on the new data
    },
  );

  // If no course found then send the response for the same
  if (!course) {
    return next(
      new ErrorHandler("Invalid course id or course not found.", 400),
    );
  }

  // Sending the response after success
  // new ApiResponse(200, true, "Course updated successfully").send(res);
  res.status(200).json({
    success: true,
    message: "Course updated successfully",
  });
});

// &---------------------------DeleteCourseById---------------------------
const deleteCourseById = AsyncHandler(async (req, res, next) => {
  // Extracting id from the request parameters
  const { id } = req.params;

  // Finding the course via the course ID
  const course = await myCourse.findById(id);

  // If course not find send the message as stated below
  if (!course) {
    return next(new ErrorHandler("Course with given id does not exist.", 404));
  }

  // Remove course
  await myCourse.findByIdAndDelete(id);

  // new ApiResponse(200, true, "Course deleted successfully").send(res);

  // Send the message as response
  res.status(200).json({
    success: true,
    message: "Course deleted successfully",
  });
});

// ^----------------------------addLectureToCourseById---------------------
const addLectureToCourseById = AsyncHandler(async (req, res, next) => {
  try {
    const { title, description } = req.body;
    const { id } = req.params;

    if (!title || !description) {
      return next(new ErrorHandler("Title and Description are required", 400));
    }

    const course = await myCourse.findById(id);

    if (!course) {
      return next(
        new ErrorHandler("Invalid course id or course not found.", 400),
      );
    }

    const lectureData = {
      title,
      description,
      lecture: {},
    };

    // Run only if user sends a file
    if (req.file) {
      try {
        const result = await cloudinary.v2.uploader.upload(req.file.path, {
          folder: "lms", // Save files in a folder named lms
          chunk_size: 50000000, //50 mb size
          resource_type: "video",
        });

        // If success
        if (result) {
          // Set the public_id and secure_url in array
          lectureData.lecture.public_id = result.public_id;
          lectureData.lecture.secure_url = result.secure_url;
        }

        // After successful upload remove the file from local storage
        fs.rm(`uploads/${req.file.filename}`);
      } catch (error) {
        // Send the error message
        return next(new ErrorHandler(error.message, 500));
      }
    }

    course.lectures.push(lectureData);

    console.log("lecture >", JSON.stringify(lectureData));

    course.numberOfLectures = course.lectures.length;

    // Save the course objec t

    await course.save();

    // new ApiResponse(
    //   200,
    //   true,
    //   "Course lecture added successfully",
    //   course,
    // ).send(res);
    res.status(200).json({
      success: true,
      message: "Course lecture added successfully",
      course,
    });
  } catch (error) {
    return next(new ErrorHandler(error.message, 500));
  }
});

// ^ ----------------------------GetLecturesByCourseId---------------------

const getLecturesByCourseId = AsyncHandler(async (req, res, next) => {
  const { id } = req.params;
  const course = await myCourse.findById(id);
  if (!course) {
    return next(
      new ErrorHandler("Invalid course id or course not found.", 404),
    );
  }
  // new ApiResponse(
  //   200,
  //   true,
  //   "Course lectures fetched successfully",
  //   course.lectures,
  // ).send(res);

  res.status(200).json({
    success: true,
    message: "Course lectures fetched successfully",
    lectures: course.lectures,
  });
});

// ^----------------------------removeLectureFromCourse--------------------

const removeLectureFromCourse = AsyncHandler(async (req, res, next) => {
  // Grabbing the courseId and lectureId from req.query
  const { courseId, lectureId } = req.query;
  console.log("courseId =>", courseId);
  // Checking if both courseId and lectureId are present
  if (!courseId) {
    return next(new ErrorHandler("Course ID is required", 400));
  }
  if (!lectureId) {
    return next(new ErrorHandler("Lecture ID is required", 400));
  }
  // Find the course uding the courseId
  const course = await myCourse.findById(courseId);
  // If no course send custom message
  if (!course) {
    return next(new ErrorHandler("Invalid ID or Course does not exist.", 404));
  }
  // Find the index of the lecture using the lectureId
  const lectureIndex = course.lectures.findIndex(
    (lecture) => lecture._id.toString() === lectureId.toString(),
  );
  // If returned index is -1 then send error as mentioned below
  if (lectureId === -1) {
    return next(new AppError("Lecture does not exist.", 404));
  }
  // Delete the lecture from cloudinary
  await cloudinary.v2.uploader.destroy(
    course.lectures[lectureIndex].lecture.public_id,
    {
      resource_type: "video",
    },
  );
  // Remove the lecture from the array
  course.lectures.splice(lectureIndex, 1);
  // update the number of lectures based on lectres array length
  course.numberOfLectures = course.lectures.length;
  // Save the course object
  await course.save();
  // new ApiResponse(200, true, "Course lecture removed successfully").send(res);

  // Return response
  res.status(200).json({
    success: true,
    message: "Course lecture removed successfully",
  });
});

module.exports = {
  createCourse,
  getAllCourses,
  updateCourseById,
  deleteCourseById,
  addLectureToCourseById,
  getLecturesByCourseId,
  removeLectureFromCourse,
};
